...
function parse_output_data(){
    ...
    #ltn,hub2,devcs,get_proj_count.sh,ltn1-vbstudio-prod-hub-2,Native,idcs-e8286c8f8f574e859d7e8da5f320b0eb,cpraaaamu-facpprod,0
    #ltn,hub2,devcs,get_proj_count.sh,ltn1-vbstudio-prod-hub-2,Native,idcs-6a102085276a4f929d9b0f205de5ac2a,ltn-vbs-test-instance-vbscanaryoc4,5
    #ltn,hub2,devcs,get_proj_count.sh,ltn1-vbstudio-prod-hub-2,Native,IDCS,Org-Count:6,TotalProjectCount:5
    for this_file in ${LOG_DIR}/${THIS_UNIQ_ID}_${INPUT_SCRIPT%.sh}.*; do
        grep -v -e "NA$" -e "FAILED" -e ",$"  $this_file | awk -F "," '{ if ($2 == "NodeName") { printf("%s,%s,%s,%s,Pod_Type,IDCS,Org_Id,Project_Count\n",$1,$2,$3,$4) } else {printf("%s,%s,%s,%s,%s,%s,%s,%s\n",$1,$5,$3,$4,$6,$7,$8,$9)}}'
        rm -f $this_file
    done | sort | uniq > $temp_csv_file
    local subject
    subject="[$(date)] VBS Org Project Count Report. Region : ${input_arguments^^}"
    local html_msg1
    html_msg1="Project Count per Org. Total Orgs = $(grep -c -v "RegionName" $temp_csv_file)"
    local html_msg2
